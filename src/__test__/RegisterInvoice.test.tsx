...
describe("App test Register Invoice", () => {
  ...
  it("add repeated product to Detalle", async () => {
    ...
    fireEvent.click(screen.getByTestId("btnGetProduct-1"));
    expect(
      await screen.findByText(/El producto ya ha sido seleccionado/i)
    ).toBeInTheDocument();
  });
  ...
  it("register invoice without detalle", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Condición Pago:/i), {
    ...
    expect(
      await screen.findByText(/Debe agregar productos/i)
    ).toBeInTheDocument();
  });
  ...
  it("submits the form with valid data", async () => {
    ...
    const nameClientInput = screen.getByLabelText(
      /Nombre Cliente:/i
    ) as HTMLInputElement;
    nameClientInput.disabled = false;
    userEvent.type(nameClientInput, "José Perez");
    nameClientInput.disabled = true;
